...
gpu_context gpu_context_make(arena* temp_arena, arena* static_arena, window* win)
{
	...
	{
		str pfif_arenas_name = str_from_fmt(temp_arena, "pfif_arenas(gpu_context)[%zu]", i);
		out.pfif_arenas[i] = arena_make(
			.capacity = Mb(1), 
			.name = strv_from_str(&pfif_arenas_name)
		);
		...
		str pfif_hc_buf_arenas_name = str_from_fmt(temp_arena, "pfif_hc_buf_arenas[%zu]", i);
		out.pfif_hc_buf_arenas[i] = gpu_arena_make(
			&out.device, 
			Mb(16), 
			host_visible_buffer_memory_type_index, 
			strv_from_str(&pfif_hc_buf_arenas_name), 
			true // host visible -> map
		); 

		
		str sw_hc_buf_arenas_name = str_from_fmt(temp_arena, "sw_hc_buf_arena[%zu]", i);		
				
		out.sw_hc_buf_arenas[i] = gpu_arena_make(
			&out.device, 
			Mb(16), 
			host_visible_buffer_memory_type_index, 
			strv_from_str(&sw_hc_buf_arenas_name), 
			true // host visible -> map
		); 
		...
		str sw_device_local_image_gpu_arena_name = str_from_fmt(temp_arena, "sw_dl_img_arenas[%zu]", i);
				
		out.sw_dl_img_arenas[i] = gpu_arena_make(
			&out.device, 
			Mb(32), // @todo: tweak around 
			render_target_memory_index, 
			strv_from_str(&sw_device_local_image_gpu_arena_name),
			false
		);

		out.st_dl_img_arenas[i] = gpu_arena_make(
			&out.device, 
			Mb(32), // @todo: tweak around 
			render_target_memory_index, 
			strv_from_str(&sw_device_local_image_gpu_arena_name),
...
void gpu_context_release(gpu_context* gpu)
{
	...
	for(uz i = 0; i < jahoda_vk_frames_in_flight; ++i)
	{
		arena_release(gpu->pfif_arenas + i);
		gpu_arena_release(&gpu->device, gpu->pfif_hc_buf_arenas + i);
		gpu_arena_release(&gpu->device, gpu->sw_hc_buf_arenas + i);
		gpu_arena_release(&gpu->device, gpu->sw_dl_img_arenas + i); 
		gpu_arena_release(&gpu->device, gpu->st_dl_img_arenas + i); 
...
bool gpu_context_try_begin_frame(gpu_context* gpu, window* win)
{
	...
	// info("pfif_arenas[%u].current: %zu", gpu->frame_index, gpu->pfif_arenas[gpu->frame_index].current);
	// info("pfif_hc_buf_arenas[%u].current: %zu", gpu->frame_index, gpu->pfif_hc_buf_arenas[gpu->frame_index].current);
	// info("commited_graphics_count[%u]: %zu", gpu->frame_index, gpu->commited_graphics_count[gpu->frame_index]);
	// info("next_graphics_buffer[%u]: %zu", gpu->frame_index, gpu->next_graphics_buffer[gpu->frame_index]);
	// info("next_transfer_buffer[%u]: %zu", gpu->frame_index, gpu->next_transfer_buffer[gpu->frame_index]);

	gpu->pfif_arenas[gpu->frame_index].current = 0;
	gpu->pfif_hc_buf_arenas[gpu->frame_index].current = 0;
	...
	if (gpu->swapchain_needs_refresh)
	{
		...
		for(uz i = 0; i < jahoda_vk_frames_in_flight; i++)
		{
			gpu->sw_dl_img_arenas[i].current = 0;
			gpu->sw_hc_buf_arenas[i].current = 0;
		}
	}
...
{
	dbg_verifyl(gpu_context, gpu->next_graphics_buffer[gpu->frame_index] <= jahoda_gpu_context_max_graphics_cmd_count, "graphics command buffer limit reached");
