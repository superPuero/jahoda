...
{	
	...
	arena* scratch_arena = gpu->pfif_arenas + gpu->frame_index;
	...
	for(uz i = 0; i < jahoda_vk_frames_in_flight; i++)
    {
        ...
        VkDeviceSize alloc = gpu_arena_push(
            gpu->sw_dl_img_arenas + gpu->frame_index, 
            mem_reqs.size, 
            mem_reqs.alignment
        );

        vk_check(vkBindImageMemory(gpu->device.handle, out.albedo_images[i],  gpu->sw_dl_img_arenas[gpu->frame_index].mem, alloc));
	...
	for(uz i = 0; i < jahoda_vk_frames_in_flight; i++)
	{
		...
		VkDeviceSize alloc = gpu_arena_push(
			gpu->sw_hc_buf_arenas + i, 
			mem_reqs.size, 
			mem_reqs.alignment
		);		

		vk_check(vkBindBufferMemory(gpu->device.handle, out.text_vbo[i], gpu->sw_hc_buf_arenas[i].mem, alloc));
		out.text_vbo_ptrs[i] = gpu->sw_hc_buf_arenas[i].mapped_ptr + alloc;
	}	
    ...
    da_foreach(&ui->nodes)
	{
		if(ui->nodes.it->type == ui_node_type_text)
		{
			...
			text_vertex_buffer verts = generate_text_vertices(
				...
				ui->nodes.it->position.x, ui->nodes.it->position.y
			);
...
{
	ctx->state = new_state;
	ctx->nodes = (ui_node_da){0};
	// @todo: change magic 128 so something meaningful	
	da_reserve(ctx->pf_arena, &ctx->nodes, 128);
}

void ui_text(ui* ctx, strv text, vec3_f32 color, vec2_f64 position)
{
	// @explain: NULL makes sure that we dont resize but only use preallocated memory
	da_append(NULL, &ctx->nodes, 
		(ui_node){
			.position = position,
			...
			.children = {0},
			.text = {
				.content = str_from_view(ctx->pf_arena, text),
				.color = color
			}
		}
	);	
...
void ui_button(ui* ctx, strv text, vec3_f32 color, vec2_f64 position)
{
	// @explain: NULL makes sure that we dont resize but only use preallocated memory
	da_append(NULL, &ctx->nodes, 
		(ui_node){
