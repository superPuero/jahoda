...
font_id font_manager_load(font_manager* manager, gpu_context* gpu, arena* static_arena, arena* scratch_arena, strv font_path)
{
	...
	VkDeviceSize image_alloc = gpu_arena_push(
		&gpu->st_dl_img_arenas[gpu->frame_index], 
		image_size, 
		mem_reqs.alignment
	);


    vk_check(vkBindImageMemory(gpu->device.handle, entry->tex.image, gpu->st_dl_img_arenas[gpu->frame_index].mem, image_alloc));
	...
	VkDeviceSize stagin_buf_alloc_offset = gpu_arena_push(
		&gpu->pfif_hc_buf_arenas[gpu->frame_index],  
		buf_mem_reqs.size, 
		buf_mem_reqs.alignment
	);

	memcpy(
		gpu->pfif_hc_buf_arenas[gpu->frame_index].mapped_ptr + stagin_buf_alloc_offset, 
	...
	vk_check(vkBindBufferMemory(
		gpu->device.handle, 
		staging_buffer, 
		gpu->pfif_hc_buf_arenas[gpu->frame_index].mem, 
		stagin_buf_alloc_offset
	));

	// @explain: getting command buffer from graphics pool, and not from transfer because 
	// only graphics capable pools supprort vkCmdCopyBufferToImage and then we can safely use same
	// buffer for transfer into device local storage because its garuanteed by vulkan spec
	// that graphics capable pools are also transfer capable
	VkCommandBuffer cmd = gpu_context_grab_graphics(gpu);
